...
export default function OleoSuspensaoPage() {
  ...
  const marca = tipoData.marcas.find((m) => m.slug === 'oleo-suspensao')!;

  function getLink(produtoNome: string) {
    const numero = slug === 'cabo' ? empresa.enderecos[1].whatsapp : empresa.enderecos[0].whatsapp;
    return gerarLinkWhatsApp(numero, { produto: `Óleo de Suspensão — ${produtoNome}` });
  }
  ...
  return (
    <div className="min-h-screen bg-zinc-950">
      <style>{`
        ...
        .os-glow { opacity: 0; transition: opacity .25s; pointer-events: none; }
        .os-card:hover .os-glow { opacity: 1; }
        .os-logo { opacity: 0.32; transition: opacity .35s, transform .35s; transform-origin: 70% 60%; }
        .os-card:hover .os-logo { opacity: 0.95; transform: translate(0, -6px) rotate(-3deg); }
        .os-cta { opacity: 0; transform: translateY(4px); transition: opacity .2s, transform .2s; }
        .os-card:hover .os-cta { opacity: 1; transform: translateY(0); }
        .os-consultor-btn {
        ...
        .os-consultor-btn:hover { transform: translateY(-2px) scale(1.02); box-shadow: 0 14px 32px -6px rgba(37,211,102,0.60); filter: brightness(1.08); }
        .os-consultor-btn:active { transform: translateY(0) scale(0.98); }
      `}</style>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-[18px]">
          {marca.produtos.map((produto) => {
            const brand = produto.brandColor ?? '#22B2D6';
            return (
              <a key={produto.id} href={getLink(produto.nome)} target="_blank" rel="noopener noreferrer"
                 className="os-card" style={{ '--brand': brand } as React.CSSProperties}>
                <div className="os-glow absolute inset-0" style={{ background: `radial-gradient(ellipse at 50% 110%, ${brand}55 0%, transparent 60%)` }} />
                {produto.logo && (
                  <div className="os-logo absolute right-0 top-0 h-full w-[55%] flex items-center justify-end pr-5">
                ...
                <div style={{ position: 'relative', padding: 22, height: '100%', display: 'flex', flexDirection: 'column', justifyContent: 'flex-end', boxSizing: 'border-box' }}>
                  ...
                  <div className="os-cta" style={{ marginTop: 14, fontSize: 13, fontWeight: 600, color: brand, display: 'flex', alignItems: 'center', gap: 6 }}>
          ...
          <ConsultorCard produto="Óleo de Suspensão" />
      </div>
    </div>
  );
}
